...
def power(token1: int | float, token2: int | float) -> float | int:
    ...
    if token2 > 1_000_000 and token1 > 1:
        raise src.exceptions.CalculatorError('inf')
    if token2 > 1_000_000 and token1 < -1:
        if token2 % 2 == 0:
            raise src.exceptions.CalculatorError('inf')
        else:
...
def division(token1: float | int, token2: float | int) -> float:
    if token2 == 0:
        raise src.exceptions.ZeroDivision(f'Деление на 0. {token1} / 0')
    if token1 % token2 == 0:
        return token1 // token2
    return token1 / token2
...
def int_division(token1: int | float, token2: int | float) -> int:
    ...
    if token2 == 0:
        raise src.exceptions.ZeroDivision(f'Деление на 0. {token1} // 0')
    raise src.exceptions.FloatNotInt(
        f'// только для целых. {token1} // {token2}'
    )
